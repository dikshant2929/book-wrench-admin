...
import Services from '../Services/VendorManagement.service';
...
export const ExpireCampaignYesPopup = (props: any) => {
 ...
 return (
        <>
            <p className="text-sm">
                Do you really want to delete <span className="font-bold">{props.data.title}</span> Vendor Management ?{' '}
            </p>
            <br />
    ...
    const onClickHandaler = (eventId: any, data: any) => {
        ...
        switch (eventId) {
            case edit.id: //Edit Campaign
                const editPath = `${VendorManagement}/edit/${encrypt.encode(JSON.stringify({ ...data, type : "Edit" }))}`;
