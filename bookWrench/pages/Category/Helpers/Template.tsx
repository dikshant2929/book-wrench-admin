...
export const ExpireCampaignYesPopup = (props: any) => {
    const onClickYes = () => {
        Services.deleteCategory(props.data.id,{}, props.data.id,(res:any) => {
            showToster({ status: 'Success', msg: res.msg || 'Category Delete Successfully' });
            popupToggler(); 
            setTimeout(() => {
                props.reloadTable();
            }, 500);
 ...
 return (
        <>
            <p className="text-sm">
                Do you really want to delete <span className="font-bold">{props.data.title}</span> Category ?{' '}
            </p>
            <br />
    ...
    const onClickHandaler = (eventId: any, data: any) => {
        ...
        switch (eventId) {
            case edit.id: //Edit Campaign
                const editPath = `${Category}/edit/${encrypt.encode(JSON.stringify({ ...data, type : "Edit" }))}`;
