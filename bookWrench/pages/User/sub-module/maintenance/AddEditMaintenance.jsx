...
const AddEditMaintenance = (props) => {
    ...
    const [fieldValue, setFieldValue] = useState({
        description: props?.maintenanceUserList[props.currentId]?.description || "",
        intervalValue: props?.maintenanceUserList[props.currentId]?.vistFrequency.value || "",
        interval: props?.maintenanceUserList[props.currentId]?.vistFrequency.interval || "",

    });


    //props.maintenanceUserList[props.currentId].maintenance.id
    ...
    useEffect(() => {
        ...
        Services.maintenanceList(
            (data) => {
                ...
                data.forEach(maintenanceList => {
                    ...
                    if (maintenanceList.id === props?.maintenanceUserList[props.currentId]?.maintenance?.id) {
        ...
        if(props?.userData?.contactAddress?.length){
            const list = [];
            props?.userData?.contactAddress.forEach(address => {
                const data = {...address, label : address.location, value: address._id};
                if(props.currentId && (address._id === props?.maintenanceUserList?.[props.currentId]?.address)){
        ...
        if(props.currentId && props?.maintenanceUserList?.[props.currentId]?.vistFrequency?.interval){
            const { id, title } = intervalList.find(({ id }) => props?.maintenanceUserList?.[props.currentId]?.vistFrequency?.interval === id);
