...
export const ExpireCampaignYesPopup = (props: any) => {
    const onClickYes = () => {
        Services.deleteSubCategory(props.data.id,{}, props.data.id,(res:any) => {
            showToster({ status: 'Success', msg: res.msg || 'Sub Category Delete Successfully' });
            popupToggler(); 
            setTimeout(() => {
                props.reloadTable();
            }, 500);
 ...
 return (
        <>
            <p className="text-sm">
                Do you really want to delete <span className="font-bold">{props.data.title}</span> Sub Category ?{' '}
            </p>
            <br />
