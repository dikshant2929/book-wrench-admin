...
export const ExpireCampaignYesPopup = (props: any) => {
    const onClickYes = () => {
        Services.deleteCustomer(props.data.id,{}, props.data.id,(res:any) => {
            showToster({ status: 'Success', msg: res.msg || 'Customer Delete Successfully' });
            popupToggler(); 
            setTimeout(() => {
                props.reloadTable();
            }, 500);
 ...
 return (
        <>
            <p className="text-sm">
                Do you really want to delete <span className="font-bold">{props.data.title}</span> Customer ?{' '}
            </p>
            <br />
    ...
    const onClickHandaler = (eventId: any, data: any) => {
        ...
        switch (eventId) {
            case edit.id: //Edit Campaign
                const editPath = `${Customer}/edit/${encrypt.encode(JSON.stringify({ ...data, type : "Edit" }))}`;
                history.push(editPath, data);
                break;
            case view.id: //Edit Campaign
                const viewPath = `${Customer}/view/${encrypt.encode(JSON.stringify({ ...data, type : "View" }))}`;
