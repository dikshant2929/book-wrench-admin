			...
			<div className='video__wrapper mt-4'>
				...
				{list.map((item, key) =>
						<div key={key}  className='flex gap-3 mb-4 w-full md:w-auto'>
							...
							<span className={`${props.disabled ? "cursor-not-allowed" : "cursor-pointer" } bg-[#FFEBE6] fill-[#E1F3EA] p-3.5 rounded-lg`} onClick={() => onItemRemove(item)}>
				...
				{!props.disabled && <div className='flex items-center gap-3'>
					<input id="video" disabled={props.disabled} className='form__input_w_height md:w-1/2' onFocus={onFocusInput} type="text" placeholder="youtube.com/watch?v=Vowek3_420o" />
